...
class Wand(
    ...
    var name: String? = if(_name == null || _name.length < 3) throw IllegalArgumentException() else _name // 이야 컴파일러가 똑똑해서 여기서 이미 null값 걸러지는 줄 알고서는 setter에서 타입을 바꿔버리네!
    ...
    var power = if(_power < 0.5 || _power > 100.0) throw IllegalArgumentException() else _power
        set(value) {
