...
* 1) 대한민국 도시 이름 모음(순서 상관 없음): 답: List 이유) 대한민국의 도시 이름은 고유 명사 이지만 지역이름이 같은 경우도 여러개 있기 때문에 불상사를 방지 하기 위해 List로 선언한다.
* 2) 10명 학생의 시험 점수 : 답 : Map 이유) 각 학생별로 점수를 파악해야 하기 때문에 학생의 이름을 Key값으로 잡고 점수를 Value값으로 가져 간다.
