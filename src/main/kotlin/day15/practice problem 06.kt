...
class PostDataSourceImpl: PostDataSource {
    ...
    override suspend fun getPost(): List<Post> {
        return Json.decodeFromString<List<Post>>(data)
    }
}
...
class PostRepositoryImpl(private val dataSource: PostDataSource): PostRepository {
    override suspend fun getPost(id: Int): Post {
        var heyGetThisData: Post = Post(-1, -1, "nothing", "nothing")
        for(value in dataSource.getPost()) {
            if(value.id == id) {
                heyGetThisData = value
                break
            }
        }
        return heyGetThisData
    }

    override suspend fun getPosts(page: Int, limit: Int?): List<Post> {
        val myPage = if(page < 1) 1 else page
