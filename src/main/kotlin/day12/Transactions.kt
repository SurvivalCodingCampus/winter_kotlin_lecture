...
fun main() {
    ...
    println("3. 케임브리지에서 근무하는 모든 거래자를 찾아서 이름순으로 정렬하여 나열하시오.")
    println(transactions.filter { it.trader.city == "Cambridge" }.sortedBy { it.trader.name }.map { it.trader.name })

    println("4. 모든 거래자의 이름을 알파벳순으로 정렬하여 나열하시오.")
    println(transactions.sortedBy { it.trader.name }.map { it.trader.name })

    println("5. 밀라노에 거래자가 있는가?")
    println(transactions.any { it.trader.city == "Milan" })

    println("6. 케임브리지에 거주하는 거래자의 모든 트랙잭션값을 출력하시오.")
    println(transactions.filter { it.trader.city == "Cambridge" }.map { it.value })

    println("7. 전체 트랜잭션 중 최대값은 얼마인가?")
    println(transactions.maxOfOrNull { it.value })

    println("8. 전체 트랜잭션 중 최소값은 얼마인가?")
    println(transactions.minOfOrNull { it.value })
}