...
class Book(
...
) : Comparable<Book>, Comparator<Book> {
    override fun equals(other: Any?): Boolean {
        other as Book
        return if (title.equals(other.title) &&
            publishedDate.format(DateTimeFormatter.ofPattern("YYYY-MM-dd"))
                .equals(other.publishedDate.format(DateTimeFormatter.ofPattern("YYYY-MM-dd")))
        ) true else false
    }
    ...
    override fun hashCode(): Int {
        return title.hashCode() * 31 + publishedDate.format(DateTimeFormatter.ofPattern("yyyy-MM-dd")).hashCode()
    }

    fun copy(): Book {
        val newTitle: String = title.toCharArray().joinToString("")
        val newAuthor: String = author.toCharArray().joinToString("")
        val newPublishedDate: LocalDateTime = LocalDateTime.of(
            publishedDate.year,
            publishedDate.month,
